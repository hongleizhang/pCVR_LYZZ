...
#include <string>
using namespace std;
#define MAX_SIZE_OF_TRAINING_SET 2000000
#define ATTR_NUM 100
...
public:
    ...
    */
    int loadData(dataToMatrix *dtm,char *file)
    {
        ...
        cout<<file<<endl;
        infile.open(file);
        while(getline(infile,tmpstrline)&&i<MAX_SIZE_OF_TRAINING_SET)
        {
            stringstream input(tmpstrline);
            if(tmpstrline!="\0"&&i>0)////由于读取文件结束符同样会继续该操作
            {
                ...
                while(getline(input,tmpstr,','))
                {
                    ...
                    input>>datatmp->attr_double[j];
                    //cout<<datatmp->attr_double[j]<<"  ";
                    j++;
                }
                p->next=datatmp;
                p=p->next;
    ...
    int loadData(dataToMatrix *dtm,char *file,int fileType)
    {
        int i=0,j=0;
        ifstream infile;
        string tmpstrline;
    ...
    */
    ...
    int loadData(dataToMatrix *dtm,char *file,int type)
    {
        ...
        while(!infile.eof()&&i<MAX_SIZE_OF_TRAINING_SET)
        {
            getline(infile,tmpstrline,',');//读取文件中一行的数据，保存为string类型
            stringstream input(tmpstrline);
            if(tmpstrline!="\0")////由于读取文件结束符同样会继续该操作
            {
